...
void Main::setup() {
  ...
  mainOLED.setupDisplay();
    ...
    xTaskCreatePinnedToCore(
      Main::OLEDloop, /* Function to implement the task */
      "OLED_loop", /* Name of the task */
      10000,  /* Stack size in words */
      (void*)&fingerPosi,  /* Task input parameter */
      (tskIDLE_PRIORITY +1),  /* Priority of the task */
      NULL,  /* Task handle. */
      1); /* Core where the task should run */
  #endif
...
void Main::loop() {
  ...
  if (comm->isOpen()){
    ...
    memcpy(fingerPosi, data.fingers, 5 * sizeof(int));
    //auto fingerPosi = fingerPos;
    ...
    comm->output(encodedString);
      ...
      if (comm->readData(received)){
        ...
        if(String(received).length() >= 5) {
           ...
           //auto servoPos = recievedData.servoValues;
           for (int i = 0; i < 5; i++){
            fingerPosi[i+11] = recievedData.servoValues[i];
          }
...
void Main::OLEDloop(void* prsty) {
  for(;;){ 
    vTaskDelay(1000);
    mainOLED.loopDisplay((int*)prsty);
    //vTaskDelay(1000);
    //delay(7500);
  }    
